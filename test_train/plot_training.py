...
def plot_diagonal_evolution(pred_file, gt_file="densities/density_series.npy", save_path="predicted_vs_gt_evolution.png"):
    ...
    plot_steps = 50  # Limit plot to first 1000 steps
    ...
    for i in range(4):
        ...
        if gt_data is not None:
            ...
            try:
                ...
                trace_gt = gt_data[:limit, spin_idx, i, i].imag
        ...
        try:
            ...
            trace_pred = pred_data[:limit, spin_idx, i, i].imag
            ax.plot(np.arange(limit), trace_pred, linewidth=2, label=f'Pred ρ[{i},{i}]', marker=".", markersize=4)
        except IndexError:
            trace_pred = pred_data[:limit, i, i].imag
            ax.plot(np.arange(limit), trace_pred, linewidth=2, label=f'Pred ρ[{i},{i}]', marker=".")
