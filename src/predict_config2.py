...
try:
    from train_benchmark import DensityMatrixLSTM, CFG
except ImportError:
...
def load_single_rho(filepath, n_basis, dtype=torch.complex128):
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Bootstrap file not found: {filepath}")
    arr = np.load(filepath)
    tensor = torch.tensor(arr, dtype=dtype)
    tensor = tensor.conj()

    if tensor.ndim == 3 and tensor.shape[0] == 2: return tensor
    elif tensor.ndim == 2 and tensor.shape == (n_basis, n_basis):
        return torch.stack([tensor, torch.zeros_like(tensor)])
    elif tensor.ndim == 3 and tensor.shape[0] == 1:
        return torch.stack([tensor[0], torch.zeros_like(tensor[0])])
    else: raise ValueError(f"Shape mismatch: {tensor.shape}")
...
def run_prediction(config_path):
    ...
    print(f"Final Shape: {full_trajectory.shape}")
